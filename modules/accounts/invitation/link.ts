...
import {
  probeMailer,
  sendAccountLink,
} from "@/modules/accounts/invitation/mailer";
...
// The single-use link, both halves (docs/permissions.md § Comptes): minting
// one and putting it in the post, then what a stranger holding it may do.
//
// Nothing here is a guard. Minting decides nothing — an administrator invites
// through admin/invite.ts, which answers for its own rung. Spending decides
// nothing either: there is no person to check, the token is the whole
// credential, and what it authorizes is exactly the one address the link was
// minted on. `requestPasswordReset` has not even that — anyone may ask, and
...
// table already knows. An administrator never sets a password; what they hand
// over is a link, and the two ways through one end here.
