...
import { isInstalled } from "@/modules/settings/installed";
...
//
// A rewrite rather than a redirect: the service hides under the one reserved
...
// the flag is irreversible, so modules/settings/installed.ts answers from memory after the
...
export async function proxy(request: NextRequest) {
  ...
  if (!installed) {
    // Already on the service: rewriting it onto itself would loop.
...
export const config = {
  matcher: [
    // Everything but Next's own static output — an installation service that
