...
**Garde**:
Le point où l'on entre dans la couche d'accès, et où le **droit** de la **personne** est tranché : une garde décide si cette personne obtient cette page ou ce formulaire, et à quel **cran**. Elle peut aussi décider si cette personne accède à **une capacité** (un panneau d'administration) plutôt qu'à une ressource nommée. Elle est donc **une seule fonction qui lit et refuse** — elle rend la ligne déjà décidée, ou lève un refus —, jamais une paire « lecture publique + contrôle à la charge de l'appelant », qu'un appelant peut oublier d'appeler. Une garde par cran : `ifReadable` pour la lecture, `assertCanWrite` pour l'écriture, `structuredPage` et `structuredForm` pour les **actions structurantes**, `editableForm` pour la définition d'un formulaire. Les gardes d'un module vivent dans un `guards.ts` niché, privé par la profondeur : `access/guards.ts`, ou le sous-dossier de la capacité quand les gardes n'en couvrent qu'une (`accounts/admin/guards.ts`). Un fichier du même module les appelle, aucun fichier d'un autre module ne les importe. Les lectures brutes (sans contrôle d'accès) y vivent aussi : la profondeur garantit que seules les gardes peuvent les appeler.
