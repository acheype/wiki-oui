...
**Couche d'accès**:
L'ensemble des fichiers autorisés à toucher `Page` et `Form` : les **gardes** de `pages` et de `forms`, et les fichiers racine de ces deux modules qui lisent ou écrivent ces tables (ADR 0025). Elle se lit dans la liste d'exceptions de la règle ESLint `wikioui/access-layer`, chaque chemin commenté. Hors d'elle, aucune fonction ne rend le contenu d'une page ni la définition d'un formulaire sans qu'un droit ait été tranché dessus ; à l'intérieur, la lecture en deux temps reste permise, et `scripts/verify-access/` la surveille au `prebuild`.
_Avoid_: Porte (l'image de l'ADR 0025, pas un nom de fichier), Repository, DAO

**Garde**:
Une fonction qui **lit et tranche dans le même appel** : elle rend la ligne déjà décidée, ou refuse. Les gardes d'un module vivent dans son `access/guards.ts`, privé par la profondeur — un fichier du même module les appelle, aucun fichier d'un autre module ne les importe. Une fonction qui ne fait qu'un seul des deux temps y vit aussi, et n'en sort pas : exportée d'un fichier racine, rien à son point d'appel ne dirait quel droit elle a sauté.
