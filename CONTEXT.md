...
**Garde**:
Le point où l'on entre dans la couche d'accès, et où le **droit** de la **personne** est tranché : une garde décide si cette personne obtient cette page ou ce formulaire, et à quelle **condition**. Elle peut aussi décider si cette personne accède à **une capacité** (un panneau d'administration) plutôt qu'à une ressource nommée. Elle est donc **une seule fonction qui lit et refuse** — elle rend la ligne déjà décidée, ou lève un refus —, jamais une paire « lecture publique + contrôle à la charge de l'appelant », qu'un appelant peut oublier d'appeler. Une garde par condition : `ifReadable` pour la lecture, `assertCanWrite` pour l'écriture, `structuredPage` et `structuredForm` pour les **actions structurantes**, `editableForm` pour la définition d'un formulaire. Les gardes d'un module vivent dans son `access/guards.ts`. La profondeur les rend privées : un fichier du même module les appelle, aucun fichier d'un autre module ne les importe. Les lectures brutes, celles qui ne tranchent rien, vivent au même endroit et n'en sortent pas. Hors du sous-dossier, n'importe quel module pourrait les appeler sans savoir qu'aucun droit n'a été vérifié.
