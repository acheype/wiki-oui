...
**Garde**:
Le point où l'on entre dans la couche d'accès, et où le **droit** de la **personne** est tranché : une garde décide si cette personne obtient cette page ou ce formulaire, et à quel **cran**. Elle est donc **une seule fonction qui lit et refuse** — elle rend la ligne déjà décidée, ou lève un refus —, jamais une paire « lecture publique + contrôle à la charge de l'appelant », qu'un appelant peut oublier d'appeler. Une garde par cran : `ifReadable` pour la lecture, `assertCanWrite` pour l'écriture, `structuredPage` et `structuredForm` pour les **actions structurantes**, `editableForm` pour la définition d'un formulaire. Les gardes d'un module vivent dans son `access/guards.ts`, privé par la profondeur — un fichier du même module les appelle, aucun fichier d'un autre module ne les importe. Une lecture qui ne tranche rien y vit aussi, et n'en sort pas : exportée d'un fichier racine, rien à son point d'appel ne dirait quel droit elle a sauté.
