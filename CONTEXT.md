...
Une balise HTML qu'un auteur peut **taper** dans sa page (`<div>`, `<details>`, `<sup>`…). Comme les composants et les classes, c'est une **liste blanche** (ADR 0002, `lib/mdx-host-elements.ts`) : elle contient ce qui met en forme de la prose, et tout le reste est retiré au rendu avec ses enfants — `script`, `style`, `iframe`, `object`, `embed`, `form`, `link`, et toute balise à laquelle personne n'a pensé. Ne concerne **que** le JSX tapé par l'auteur : le HTML que markdown produit lui-même (tableau, liste, case à cocher) ne passe pas par ce chemin et n'est jamais filtré. Une balise refusée est signalée à l'enregistrement. Y ajouter une balise est un geste d'opérateur (on édite le code), comme pour une famille de classes (ADR 0011).
...
**Embed (`<Embed>`)**:
Composant intégré affichant la page d'un autre site dans la page (`url`, `title`, `ratio`). Il porte l'usage que `<iframe>` servait avant la liste blanche des balises : la balise brute est refusée parce qu'elle mêle deux choses sans rapport — un `src` cross-origin, muré par la politique de même origine, et un `srcdoc` qui hérite de **notre** origine et y exécute ses scripts. Passer par un composant rend la moitié sûre atteignable sans l'autre, et impose ce qu'une balise tapée à la main ne garantit jamais : `https` seul, `sandbox` sans `allow-top-navigation` (sinon le site embarqué redirige l'onglet du lecteur), et un `title` pour les lecteurs d'écran.
_Avoid_: iframe (la balise n'est pas rendue), oEmbed / unfurl (aucune métadonnée n'est récupérée)
...
**v0.3 (en cours)** : formulaires & fiches (ADR 0014/0015, spec [`docs/forms.md`](docs/forms.md)) — FormBuilder (pages spéciales `formulaires` et `fiches`), 14 types de champs, saisie via `<EntryForm>`, rendu par défaut + gabarit MDX optionnel, renderer de champs partagé avec le ComponentBuilder, Zod comme contrat runtime, API de redimensionnement d'images. `<EntriesView>` (vues riches des fiches) : plus tard. En cours de route (2026-07-17), colmatage du bac à sable (ADR 0002) : liste blanche de balises HTML, refus de `dangerouslySetInnerHTML`, et composant `<Embed>` reprenant l'usage d'`<iframe>`.
