...
_Avoid_: handler (pour un service) ; réserver un segment d'URL de niveau 1 par service (un seul segment réservé : `api`, qui abrite aussi l'écran d'installation — ADR 0028)
...
**Page spéciale**:
Une page à **slug réservé**, créée au seed de la base, **non supprimable, non renommable, mais éditable** (comme n'importe quelle page : flux d'édition normal, rendu MDX normal). C'est le seul trait qui la définit ; « alimenter le layout » n'est qu'une propriété de certaines d'entre elles. Les pages spéciales : les 5 pages de layout (`page-titre`, `page-menu-haut`, `page-rapide-haut`, `page-header`, `page-footer`), `page-principale` (l'accueil, cible de la redirection `/` → `/page-principale` ; contenu ordinaire), `aide-memoire`, depuis la v0.3 les écrans d'administration des formulaires : `formulaires` (rend `<FormsAdmin>`) et `fiches` (rend `<EntriesAdmin>`), et depuis la v0.5 `gerer-utilisateurs` (rend `<UsersAdmin>` et `<GroupsAdmin>`) ainsi que les quatre écrans de comptes : `connexion`, `inscription`, `mot-de-passe-oublie` et `invitation` (ADR 0028). Le menu n'a pas de rendu spécial : le contenu par défaut de `page-menu-haut` appelle le composant intégré `<Menu>`, et celui de `page-rapide-haut` expose les 5 pages de layout derrière un bouton roue crantée (`<Menu>` + `<Button>`).
_Avoid_: Page seedée, template, page système, fragment

**Écran**:
Une interface d'application servie par le wiki : se connecter, administrer les formulaires, gérer les utilisateurs. **Un écran est toujours une page** (ADR 0028) — une page spéciale dont le contenu appelle un composant intégré, jamais une route ajoutée dans `app/`, qui prélèverait un slug sur l'espace de noms des pages sans le dire. Ce dont il a besoin voyage dans la **query string** (`?suite=`, `?jeton=`, `?formulaire=`), jamais dans un segment : derrière le slug d'une page, un segment est un handler. Seule exception, le segment réservé `/api`, qui abrite les deux écrans ne pouvant pas être des pages — l'aperçu du ComponentBuilder et l'écran d'installation (`/api/installation`, présenté par réécriture depuis n'importe quelle adresse tant que le wiki n'est pas installé, ce qui laisse le slug `installation` libre).
_Avoid_: Route dédiée, écran sous `/admin`, page « système »
...
**v0.5** : utilisateurs & droits (spec grillée le 2026-07-30 : [`docs/permissions.md`](docs/permissions.md), ADR 0023 à 0028) — authentification par BetterAuth (plugin `username`, connexion par email *ou* identifiant), autorisation entièrement WikiOui : trois niveaux d'accès, groupes imbriqués, et un droit = une **portée** (*tout le monde* · *les personnes connectées* · *seulement*) complétée d'une liste, le propriétaire et les administrateurs étant toujours autorisés. Les droits se posent à quatre étages — page/fiche, formulaire (créer une fiche + deux défauts), champ (fusion à l'écriture, jamais de remplacement), configuration — et les défauts se **recopient** à la création, sans jamais se lier (ADR 0026). Deux pages spéciales de plus, `gerer-utilisateurs` (comptes, invitations par lot, groupes et leur imbrication expliquée) et `gerer-pages` (recherche, filtres, lot « Remplacer » ou « Donner accès »). S'y ajoutent le handler **`/{slug}/raw`** (MDX ou JSON brut, champs restreints retirés), la propriété d'auteur `hideIfNoAccess` sur les liens, boutons et iframes, l'**écran d'installation** à drapeau irréversible (ADR 0027) et la première colonne de la table `Settings`. Les écrans de comptes sont eux aussi des **pages spéciales** — `connexion`, `inscription`, `mot-de-passe-oublie` et `invitation`, dont le jeton voyage en `?jeton=` (ADR 0028) — l'écran d'installation lui-même n'ajoutant aucune route de niveau 1, puisqu'il vit sous `/api` et se présente par réécriture. Le contrôle passe par une **couche d'accès unique gardée par ESLint** (ADR 0025), jamais par RLS ni par une extension Prisma.
