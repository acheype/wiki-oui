...
**Slug**:
L'identifiant textuel d'une page, et sa seule identité : il est tapé directement dans l'URL (pas généré depuis un titre). Format `^[a-z0-9]+(?:-[a-z0-9]+)*$` (minuscules, chiffres, tirets ; pas de CamelCase à la YesWiki). C'est le niveau 1 de l'URL : `https://site/{slug}`. Les majuscules sont normalisées par redirection (`/Ma-Page` → `/ma-page`) ; un slug hors motif renvoie *not found*. Trop technique pour un utilisateur, le mot *slug* est traduit par **identifiant** dans les interfaces. En développement, on dit *slug* pour ne pas le confondre avec les identifiants UUID de la base. **Renommable** a posteriori par l'action « Changer l'adresse » (administrateur, confirmation chiffrée) : changer le slug change l'adresse de la page, et l'ancienne cesse d'exister. Toutes les références internes sont réécrites, historique compris, pour qu'aucun lien du wiki ne casse. Le wiki ne peut rien réécrire hors de lui : un favori, ou un lien posé sur un autre site, pointe toujours vers l'ancienne adresse et cesse donc de fonctionner — assumé, et annoncé dans le dialogue de confirmation.
_Avoid_: Id (réservé aux identifiants UUID de la base), nom de page, permalink ; « slug » dans une interface
...
**Module**:
Un dossier `modules/<concept>/` qui regroupe tout le code d'un concept du domaine, un par nom de `CONTEXT.md` — `app/` ne garde que les routes (ADR 0029). Son **interface** est ce que montre `ls modules/<concept>/` : un fichier à sa racine s'importe depuis n'importe quel autre module, un fichier de sous-dossier ne s'importe que depuis l'intérieur du sien — sauf `ui/`, que `app/` seul peut composer depuis l'extérieur. La profondeur porte donc la visibilité, gardée par une règle ESLint plutôt que par convention. On cherche à en faire un **module profond** (ADR 0030) : beaucoup de comportement derrière peu d'interface, donc le moins de fichiers et d'exports possible à la racine. Une fonction qu'aucun autre module n'importe descend dans un sous-dossier. Le `wiki-components/` d'un module fait exception lui aussi : ce sous-dossier est **public** (voir Registre de composants), car le registre de composants l'atteint depuis l'extérieur. La liste des modules qui en possèdent un est déclarée dans `modules/authoring/registry/sources.ts`.
_Avoid_: Package, couche, dossier (trop générique — un module a une interface, un dossier quelconque n'en a pas) ; confondre la **profondeur de dossier**, qui dit qui importe (ADR 0029), et le **module profond**, qui dit combien on expose (ADR 0030)

**Couche d'accès**:
L'ensemble des fichiers autorisés à toucher `Page` et `Form` : les **gardes** de `pages` et de `forms`, et les fichiers racine de ces deux modules qui lisent ou écrivent ces tables (ADR 0025). Elle se lit dans la liste d'exceptions de la règle ESLint `wikioui/access-layer`, chaque chemin commenté. Hors d'elle, aucune fonction ne rend le contenu d'une page ni la définition d'un formulaire sans qu'un droit ait été tranché dessus. À l'intérieur, une lecture peut encore **se séparer de sa garde** — lire la ligne d'abord, appeler la garde ensuite, en deux instructions (ex. `content.ts` lit la page, puis appelle `ifReadable`) : c'est admis parce que le fichier ne publie que le résultat des deux. Ce qui l'admet le vérifie : `scripts/verify-access/` remonte, au `prebuild`, le graphe d'appels de chaque lecture exportée de la couche jusqu'à `canRead`, `canWrite` ou `isAdmin`, et fait échouer le build pour celle qui n'y arrive pas.
_Avoid_: Porte (l'image de l'ADR 0025, pas un nom de fichier), Repository, DAO ; « deux temps » pour cette séparation (réservé au couple SQL puis mémoire de `docs/permissions.md`)

**Garde**:
Le point où l'on entre dans la couche d'accès, et où le **droit** de la **personne** est tranché : une garde décide si cette personne obtient cette page ou ce formulaire, et à quelle **condition**. Elle peut aussi garder une **page système** d'administration : `gerer-utilisateurs`, `gerer-pages`. Aucune ressource n'y est nommée, et la question devient « cette personne est-elle administratrice ? ». Elle est donc **une seule fonction qui lit et refuse** — elle rend la ligne déjà décidée, ou lève un refus —, jamais une paire « lecture publique + contrôle à la charge de l'appelant », qu'un appelant peut oublier d'appeler. Une garde par condition : `ifReadable` pour la lecture, `assertCanWrite` pour l'écriture, `structuredPage` et `structuredForm` pour les **actions structurantes**, `editableForm` pour la définition d'un formulaire. Les gardes d'un module vivent dans son `access/guards.ts`. La profondeur les rend privées : un fichier du même module les appelle, aucun fichier d'un autre module ne les importe.
_Avoid_: Check, garde-fou, vérification (une garde refuse, elle ne signale pas), queries.ts (nom retiré pour un fichier de gardes : il dit la mauvaise moitié de ce que le fichier fait — `modules/permissions/groups-queries.ts` le garde, étant une porte publique et non un fichier de gardes)
...
**Action**:
Ce qu'une personne déclenche sur une page ou une fiche : éditer, poser des tags, restaurer une révision, supprimer, modifier les droits, transmettre la propriété, changer l'adresse. Chacune s'arrête à une **condition** — le droit d'écriture, le propriétaire ou administrateur (les **actions structurantes**), l'administrateur seul (l'adresse) — et cette condition suit la **portée de l'effet**, pas une hiérarchie de personnes : qui peut écrire peut de toute façon vider une page, mais l'historique survit à un blanchiment, pas à une suppression. Une action indisponible est **absente** de la barre, jamais grisée : une offre impossible n'informe personne. En code, l'ensemble des actions ouvertes à une personne sur une page est un `PagePermissions`.
_Avoid_: Geste (abandonné en v0.5), cran (abandonné le 2026-08-29 : métaphore de graduation, illisible pour qui n'a pas la clé), opération, commande ; « action » pour désigner un handler de page, un composant ou une Server Action
