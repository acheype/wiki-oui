...
- **Les commandes séparent la vitesse du coût.** `pnpm test` ne lance que `unit` (rapide, quotidien) ; `pnpm test:browser` ne lance que `browser` ; `pnpm test:all` lance les deux ; `pnpm test:watch` observe. Le coût du binaire de navigateur ne se paie que si son projet tourne.
...
- **La boucle quotidienne reste rapide.** `pnpm test` ne démarre aucun navigateur ; les 47 tests Node et les tests jsdom gardent leur vitesse.
