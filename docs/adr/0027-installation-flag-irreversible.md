# Le service d'installation est gardé par un drapeau irréversible, pas par l'absence d'administrateur

*(Amendé le 2026-08-22 : « écran » a quitté le glossaire ; l'installation n'a jamais été une page système — elle vit sous `/api`, voir ADR 0028 — donc « service », déjà le terme correct, remplace « écran » ici sans changer le raisonnement.)*

Tant que le wiki n'a **jamais été installé**, toute route mène au service d'installation, qui crée le compte administrateur initial. La condition est un drapeau posé une fois pour toutes (`Settings.installedAt`), **pas** le test « existe-t-il un administrateur ? ».

## Contexte

Un test réversible ferait du service un **chemin d'escalade** : quiconque parviendrait à vider `@Admins` — par un bug d'autorisation, non par le produit, où retirer le dernier administrateur est refusé — n'aurait plus qu'à recharger la page d'accueil pour se nommer administrateur. Le drapeau transforme cette porte en porte à sens unique.
...
**Un lien d'invitation imprimé dans les logs.** Aucun secret en environnement, mais il faut ouvrir les logs — ce que le service d'installation existe précisément pour éviter.

Le service, lui, tourne **dans Next** avec BetterAuth au complet : le seed reste un script de données pur, et ni variable ni log ne sont nécessaires.
...
- Le compte créé porte le nom **Wiki Admin** et l'identifiant **`wiki-admin`**, imposés — convention identique sur toutes les installations, qui permet à un support de dire « connectez-vous en `wiki-admin` » sans rien demander. Rien n'est figé pour autant : le nom se change depuis le profil, l'identifiant par l'action de renommage, et le compte se supprime une fois un autre administrateur en place.
- Le service n'a **pas d'adresse à lui** : il vit sous le segment réservé (`/api/installation`) et le proxy y **réécrit** n'importe quelle adresse tant que le drapeau manque (ADR 0028). Le slug `installation` reste donc libre pour une page, qui s'ouvre normalement une fois le wiki installé.
