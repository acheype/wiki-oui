...
- Le seed reste **exempt de BetterAuth** : l'écran d'installation (ADR 0027) crée le compte administrateur depuis Next, où la bibliothèque est complète. Le `node_modules` isolé de `docker/deploy-tools` (ADR 0021) n'a donc pas à l'embarquer, et `lib/auth.ts` n'a pas de contrainte d'importabilité hors Next.
