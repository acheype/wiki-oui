...
Tant que le wiki n'a **jamais été installé**, toute route mène à l'écran d'installation, qui crée le compte administrateur initial. La condition est un drapeau posé une fois pour toutes (`Settings.installedAt`), **pas** le test « existe-t-il un administrateur ? ».
...
- L'écran n'a **pas d'adresse à lui** : il vit sous le segment réservé (`/api/installation`) et le proxy y **réécrit** n'importe quelle adresse tant que le drapeau manque (ADR 0028). Le slug `installation` reste donc libre pour une page, qui s'ouvre normalement une fois le wiki installé.
