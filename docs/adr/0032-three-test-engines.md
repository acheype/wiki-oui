...
- **Auth par le vrai flux d'installation.** Le seed reste des données pures (ADR 0027, aucun import `better-auth`) : le compte `wiki-admin` ne naît que par le service d'installation, qui tourne dans Next. Un projet Playwright `setup` (`e2e/install.setup.ts`) joue donc l'installation réelle avant le parcours, dont il est la dépendance. Aucun `storageState` : le parcours se connecte **explicitement**, comme une personne.
