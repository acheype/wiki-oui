...
- **Un identifiant pas encore en base** (nouveau champ, ou nouveau formulaire) est un **input visible**, dérivé automatiquement (du libellé pour un champ, du nom pour le formulaire) jusqu'à la première modification par l'utilisateur ; **vidé, il redevient dérivé** (convention du slug de fiche). Plus de révélation par chevron ni de bouton « Personnaliser ».
