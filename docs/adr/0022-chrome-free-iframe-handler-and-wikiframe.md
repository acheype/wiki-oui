...
- Contenu : un **slug interne** seul, validé par `isValidSlug`. Ferme la forge `?modale=https://site-piege` ; une cible externe garde l'iframe sandboxée (`ModalLink` + `WikiFrame`), **sans URL**.
...
2. **La fédération inter-instances.** `/{slug}/iframe` inclut `WikiFrameResizeEmitter` (`modules/pages/ui/wiki-frame-emitter.tsx`), qui publie sa hauteur par `postMessage` : une autre instance WikiOui embarque une de nos pages en s'auto-dimensionnant, sans configuration d'aucun côté.

La route rend désormais la page **entière** (titre compris) : `?title=hidden`, `data-wiki-title` et le rappel `onTitle` — qui traversaient une frontière de document — ont disparu avec le dernier client qui les utilisait.
...
- **Sécurité** : une cible **externe** reste sandboxée ([ADR 0002](0002-mdx-sandbox.md) : `sandbox` sans `allow-top-navigation`, `referrer-policy`, http(s) seul). Une cible **interne** est same-origin et non sandboxée (elle exécute nos composants client). La hauteur `postMessage` n'est pas sensible → `targetOrigin: "*"` côté émetteur, l'origine du parent étant inconnue d'avance.
