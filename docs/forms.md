...
- **`data` est écrit dans l'ordre des champs du formulaire**, `title` en premier (`orderedEntryData()`, `lib/form-descriptor.ts`) — un confort à l'inspection directe de la base, la garantie utile étant celle que tient `/{slug}/raw` en la reconstruisant à la lecture ([`permissions.md`](permissions.md) § `/{slug}/raw`) : `Revision.data` est du `jsonb`, qui ne préserve pas l'ordre d'écriture.
