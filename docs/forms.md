...
- **`content`, `form-id` et `metadata` sont des identifiants de champ refusés à l'enregistrement** (`formAuthoringIssues()`, `lib/form-descriptor.ts`) : ce sont les noms que `/{slug}/raw` écrit lui-même à côté des valeurs d'une fiche ([`permissions.md`](permissions.md) § `/{slug}/raw`) — un champ qui les porterait entrerait en collision avec eux. `title` n'a pas besoin de la même règle : le méta-schéma le réserve déjà au seul champ de type Titre.
