...
- **Descripteur ComponentBuilder → YAML seul, composants sans contrat, vérifié par signature** (validé le 2026-07-12, ADR 0013 ; conception **non encore implémentée**). Les défauts quittent le `.tsx` (l'export `xxxDefaults` forçait le composant serveur, cf. le bug du menu commit `67ffd53`) pour le YAML ; le composant devient du React ordinaire, client ou serveur. La cohérence YAML ↔ composant (champ ∈ props, obligatoire, type, dérive du `default`, type de `value`) est vérifiée en parsant la **source** du composant (`ts-morph`, *devDependency*), au build (`prebuild` bloquant) et en dev (overlay d'erreur), jamais au runtime de prod. Détail : [`component-builder.md`](component-builder.md). Changement compagnon (étape séparée) : route d'icônes `GET /api/icons/[id]` + `<Icon>` client, pour que les composants à icône puissent aussi être client.
